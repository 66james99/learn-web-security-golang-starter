...
func (handler *Handler) UpdateEmail(responseWriter http.ResponseWriter, request *http.Request) {
	...
	currentPassword, pwErr := httpx.FormValue(request, "currentPassword")
	if pwErr != nil {
	...
	if !passwords.Verify(currentPassword, current.User.PasswordHash) {
		...
		handler.errorPage(responseWriter, http.StatusForbidden, "Forbidden", "Re-enter your current password to change your email.")
		return
	}
